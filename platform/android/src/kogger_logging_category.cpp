...
void KoggerLoggingCategoryManager::setFilterRulesFromSettings(const QString &commandLineLoggingOptions)
{
    ...
    for (const QString &fullCategoryName : settings.childKeys()) {
        ...
        qCDebug(KoggerLoggingCategoryRegisterLog) << "Setting filter rule for saved settings"
                << fullCategoryName << parentCategory << childCategory << settings.value(fullCategoryName).toBool();
