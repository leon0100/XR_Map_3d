...
int32_t TileGeovisEarthProvider::heightToTileZ(const float height) const
{
    double z = std::log2(GEOVISEARTH_TILE_CONSTANT / height);
    z = std::max(0.0, std::min(z, 21.0));
    return static_cast<int>(z);
}

int32_t TileGeovisEarthProvider::lonToTileX(const double lon, const int z) const
{
    if (z < 0 || z > 21) {
...
std::tuple<int32_t, int32_t, int32_t> TileGeovisEarthProvider::lonToTileXWithWrapAndBoundary(const double lonStart, const double lonEnd, const int z) const
{
    if (z < 0 || z > 21) {
        qWarning() << "Invalid zoom level:" << z;
        return {-1, -1, -1};
...
int32_t TileGeovisEarthProvider::latToTileY(const double lat, const int z) const
{
    if (z < 0 || z > 21) {
        qWarning() << "Invalid zoom level:" << z;
        return -1;
...
map::TileInfo TileGeovisEarthProvider::indexToTileInfo(map::TileIndex tileIndx, map::TilePosition pos) const
{
    TileInfo info;
    GeoBounds bounds;
