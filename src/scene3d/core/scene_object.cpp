...
void SceneObject::RenderImplementation::updateBounds()
{
    ...
    float z_max{ !qIsFinite(m_data.first().z()) ? 0.f : m_data.first().z() };
    float z_min{ z_max };
    float x_max{ !qIsFinite(m_data.first().x()) ? 0.f : m_data.first().x() };
    float x_min{ x_max };
    float y_max{ !qIsFinite(m_data.first().y()) ? 0.f : m_data.first().y() };
    float y_min{ y_max };
