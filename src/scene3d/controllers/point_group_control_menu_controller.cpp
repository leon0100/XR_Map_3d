...
{
    qDebug() << "onCoordSpinBoxValueChanged...........";
    if(row < 0)
        return;
    ...
    m_pointListModel->item(row)->setData(QString("x: %1, y: %2, z: %3").arg(coord.x()).arg(coord.y()).arg(coord.z()),Qt::DisplayRole);
...
{
    qDebug() << "onPointColorDialogAccepted........";
    if(!m_graphicsSceneView)
        return;
